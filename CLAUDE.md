...
3b. **Pricing modes & buy verdict** (`LOT_MODES`, `setLotMode`, `computeVerdict`, `ebayLookup`): the bulk evaluator has a Games / Movies / Books / Music mode bar (persisted in `localStorage.lot_mode`).
   ...
   - **Platform inference**: Live Text lists badges/serials as a separate column, so `parseSpineDump` also votes on platform from every badge and serial (`platformFromSerial`: SLES/SCES >=50000 PS2, BLES/BCUS PS3, CUSA PS4, PPSA PS5, ULES PSP) and gives the dominant one (>=70%) to titles without one. Logo misreads handled: "P53" PS3, "XBOX 30U"/"XBX 360" Xbox 360, "Blu-ray Disc" is a platform line. Franchise prefix lines ("Tom Clancy's", "Disney") are joined to the next line (`PREFIX_LINE_RX`).
   - **Verdict safeguards** (tested on 6 real shop photos, 2026-09-23): eBay is always searched with the matched CeX name (`buildEbayQuery`/`ebayQueryName`) and re-queried when the user swaps a match (`refreshItemEbay`). The Worker returns up to 50 listings; `relevantEbay` keeps only listings containing >=60% of the title's key words and recomputes low/p25/median. eBay sell is capped at CeX's own shelf price when it is more than 2x or $15 above it (bundles/special editions). Fewer than 5 relevant listings can't make a BUY. Verdict `verify` ("Confirm match") replaces BUY/Marginal for amber matches, one-word reads matched to longer titles, matches that add edition words (Freedom/Collector's/Limited...), and in Games/Movies any line CeX couldn't identify. Catalog ranking prefers the plainer title when scores are close. The app sends `v=2` on eBay calls to bypass browser-cached responses after Worker changes; bump it when the Worker response shape changes.
